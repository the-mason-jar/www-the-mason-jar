...
docpadConfig = {
    ...
    collections:
        ...
        allCocktails: (database) ->
            database.findAll({relativeOutDirPath:/cocktails[\/\\]\w+/}, [date:-1]).on('add', (document) ->
                document.setMetaDefaults(layout:'recipe')  unless document.get('layout')
            )

        allInfusions: (database) ->
            database.findAll({relativeOutDirPath:/infusions[\/\\]\w+/},[date:-1]).on('add', (document) ->
                document.setMetaDefaults(layout:'recipe')  unless document.get('layout')
            )

        allExtras: (database) ->
            database.findAll({relativeOutDirPath:/extras[\/\\]\w+/},[date:-1]).on('add', (document) ->
                document.setMetaDefaults(layout:'recipe')  unless document.get('layout')
            )
