...
(function() {
  ...
  DocPad = (function(_super) {
    ...
    DocPad.prototype.events = ['extendTemplateData', 'extendCollections', 'docpadLoaded', 'docpadReady', 'docpadDestroy', 'consoleSetup', 'generateBefore', 'populateCollectionsBefore', 'populateCollections', 'generateAfter', 'parseBefore', 'parseAfter', 'contextualizeBefore', 'contextualizeAfter', 'renderBefore', 'renderCollectionBefore', 'render', 'renderDocument', 'renderCollectionAfter', 'renderAfter', 'writeBefore', 'writeAfter', 'serverBefore', 'serverExtend', 'serverAfter', 'notify'];
    ...
    */
    ...
    DocPad.prototype.loadFiles = function(opts, next) {
      var collection, config, database, docpad, locale, slowFilesObject, slowFilesTimer, _ref1;
      if (opts == null) {
        opts = {};
      }
      docpad = this;
      ...
      if ((_ref1 = opts.progress) != null) {
        _ref1.step("loadFiles (preparing)").total(1).setTick(0);
      }
      ...
      docpad.emitSerial('parseBefore', {
        ...
        tasks = new TaskGroup().setConfig({
          ...
          if ((_ref2 = opts.progress) != null) {
            _ref2.step("loadFiles (postparing)").total(1).setTick(0);
          }
        ...
        if ((_ref2 = opts.progress) != null) {
          _ref2.step('loadFiles').total(collection.length).setTick(0);
        }
        collection.forEach(function(file) {
          ...
          return tasks.addTask(function(complete) {
            ...
            return file.action('load', function(err) {
              var fileIgnored, fileParse, _ref3;
    ...
    DocPad.prototype.renderFiles = function(opts, next) {
      ...
      renderFile = function(file, next) {
        if (file.get('dynamic') || !file.get('render') || !file.get('relativePath')) {
    ...
    DocPad.prototype.generate = function(opts, next) {
      ...
      opts.templateData || (opts.templateData = docpad.getTemplateData());
      ...
      addGroup('Determine files to render', function(addGroup, addTask) {
        if (opts.partial === false) {
          return addTask('Add all database models to render queue', function() {
            return opts.collection != null ? opts.collection : opts.collection = new FilesCollection().add(database.models);
          });
        } else {
          return addTask('Add only changed models to render queue', function() {
            return opts.collection != null ? opts.collection : opts.collection = new FilesCollection().add(database.findAll({
              $or: {
                mtime: {
      ...
      addTask('Load Files', function(complete) {
        var model;
        ...
        return docpad.loadFiles(opts, function(err) {
          ...
          opts.collection.reset(opts.collection.reject(function(file) {
            return file.get('dynamic') || file.get('write') === false;
          }));
